...
app.get('/hotspotStates', async (req, res) => {
    ...
    try {
        const data = await connection.aggregate([
            {"$match": {"$expr": {"$gt": [{"$round": [{"$divide": [{ "$subtract": ['$infected', '$recovered'] },'$infected']}, 5]}, 0.1]}}},
...
app.get('/healthyStates', async (req, res) => {
    ...
    try {
        const data = await connection.aggregate([
            {"$match": {"$expr": {"$lt": [{ "$round": [{ "$divide": ['$death','$infected']}, 5]}, 0.005]}}},
            {"$project": { "_id": 0,"state": 1,"mortality": {"$round": [{"$divide": ['$death','$infected']}, 5]}}}
        ])
